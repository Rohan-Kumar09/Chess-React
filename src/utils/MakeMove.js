import { FindValidMoves, isKingSafeAfterMove, inCheckmate } from './ValidMoves.js'; // function for finding valid piece moves
...
export function MakeAMove(selectedPiece, goToRow, goToCol, setBoard, board, setSelectedPiece, turn, setTurn, audio){
    ...
    simulatedBoard[selectedPiece.row][selectedPiece.col] = {emoji: ' ', name: 'empty'};
    simulatedBoard[goToRow][goToCol] = {emoji: selectedPiece.piece, name: selectedPiece.name};

    // Ensure the move doesn't leave the king in check.
    if (!isKingSafeAfterMove(simulatedBoard, turn)) {
        setSelectedPiece({emoji: ' ', row: -1, col: -1, name: 'empty'});
        return;
    }
